...
def Executor():
    ...
    def wrapper(method):
        ...
        def inner(self, *args, **kwargs):
            ...
            # start_time = time.time()
            # log.info('Task(%s) start at %s' % (method.__name__,start_time), self.order)
            # res = os.popen('ping www.baidu.com')
            # res = subprocess.Popen(self.order)
            # res = subprocess.run(self.order)
            ...
            # log.info('Executor res',type(res),res.stdout.read(),res.returncode,res.stdout,res.terminate(),res.wait())
...
def Executor_v2():
    '''执行shell命令的类方法装饰器:
        - 用于类方法 可接收参数self
        - 对比v1版本的Executor 将添加命令前缀order_prefix_v2的工作在装饰器内做了统一处理
    '''
    def wrapper(method):
        @functools.wraps(method)
        def inner(self, *args, **kwargs):
